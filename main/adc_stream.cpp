...
constexpr size_t kPackedHeaderSize = 12;
...
void configure_adc(adc_continuous_handle_t handle, uint32_t sample_rate,
                   uint8_t bit_width, uint8_t channel_mask,
                   uint8_t attenuation)
{
    ...
    for (size_t i = 0; i < kMaxChannels; ++i) {
        ...
        pattern[channel_count].bit_width = bit_width;
        ++channel_count;
    }
                    ...
                    uint8_t channel_mask, uint8_t *output)
{
    ...
    uint8_t channel_count = 0;
    for (size_t i = 0; i < kMaxChannels; ++i) {
        if ((channel_mask & (1u << i)) != 0) output[3 + channel_count++] = kGpios[i];
    }
    output[2] = channel_count;
    for (size_t i = channel_count; i < kMaxChannels; ++i) output[3 + i] = 0;
    ...
    output[9] = first_channel_index;
    output[10] = sample_count & 0xff;
    output[11] = (sample_count >> 8) & 0xff;
                                       ...
                                       uint8_t attenuation)
{
    ...
    configure_adc(handle, sample_rate, bit_width, channel_mask, attenuation);
    return handle;
}
} // namespace
...
[[noreturn]] void scope::AdcStream::run() noexcept
{
    ...
    adc_continuous_handle_t handle = initialize_adc(active_sample_rate,
        active_bit_width, active_channel_mask, active_attenuation);
    ...
    while (true) {
        ...
        if (requested_sample_rate != active_sample_rate ||
            requested_bit_width != active_bit_width ||
            ...
            ESP_LOGI(tag, "ADC: %" PRIu32 " Sa/s, %u bits, mask 0x%02x, atten %u",
                     active_sample_rate, active_bit_width, active_channel_mask,
                     active_attenuation);
        }
